...
model.load_state_dict(torch.load(folder_saved_model + ''),strict=False)
...
list_kcs = [[sol for sol in value['descs']] for key,value in data_questions.items()]
questions = ['[Q] ' + q for q in list_questions]
kcs = [['[S] ' + step for step in sol_steps] for sol_steps in list_kcs]

question_embeddings = text_to_embeddings(questions)
flat_descs = [step for sublist in kcs for step in sublist]
flat_desc_embeddings = text_to_embeddings(flat_descs)
...
for steps in kcs:
    ...
    sol_step_embeddings.append(flat_desc_embeddings[start_idx:end_idx])
    start_idx = end_idx

np_question_embeddings = question_embeddings.cpu().detach().numpy()
np_desc_embeddings = []
for i in range(len(sol_step_embeddings)):
    np_desc_embeddings.append(sol_step_embeddings[i].cpu().detach().numpy())

np_desc_embeddings_mean = []
for i in range(len(np_desc_embeddings)):
    np_desc_embeddings_mean.append(np_desc_embeddings[i].mean(axis=0))

dict_emb = {}
for i in range(len(np_question_embeddings)):
    emb_q = np_question_embeddings[i].copy().reshape(1,-1)
    emb_sol = np_desc_embeddings_mean[i].copy().reshape(1,-1)
    emb = (emb_q + emb_sol)/2
