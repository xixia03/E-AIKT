...
class EaiKTNet(nn.Module):
    def __init__(self, num_q, num_c, emb_size, n_blocks, dropout, d_ff=256,
        ...
        if self.num_q > 0:
            ...
            self.beta.weight.requires_grad = False 
    ...
    def forward(self, q, c, r):
        ...
        return preds, c_reg_loss, interpret_dict
...
class EaiKT(QueBaseModel):
    ...
    def predict_one_step(self, data, return_details=False, process=True):
        ...
        y, reg_loss, interpret_dict = self.model(data_new['cq'].long(), data_new['cc'].long(), data_new['cr'].long())

        y = y[:, 1:]

        if return_details:
            data_new.update(interpret_dict) 
