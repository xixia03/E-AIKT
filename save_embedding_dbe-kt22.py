...
model.load_state_dict(torch.load(folder_saved_model + '/bert_finetuned_epoch_**.bin'))
...
for idx, value in enumerate(data_questions.values()):
    ...
    dict_emb[value['id']] = norm_emb.tolist()

save_path = os.path.join(embeddings_save_folder, 'qid2content_desc_avg_emb_act.json')
